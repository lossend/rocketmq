...
import org.apache.rocketmq.proxy.lifecycle.warmup.WarmupBarrier;
...
public final class ProxyGracefulLifecycleWiring {
    ...
    /**
     * Drives startup warmup from a {@link WarmupRegistry} with no runtime readiness probes
     * installed (equivalent to an empty {@link ReadinessProbeRegistry}). Uses the scheduler
     * captured at {@code createCoordinator}.
     *
     * @param warmupRegistry the registered one-time startup warmup tasks
     * @param config         supplies the warmup timeout budget
     * @param onReady        published exactly once when the warmup barrier completes
     */
    public void startWarmup(WarmupRegistry warmupRegistry, ProxyConfig config, Runnable onReady) {
        startWarmup(warmupRegistry, new ReadinessProbeRegistry(), config, onReady);
    }

    /**
     * Drives startup warmup from a {@link WarmupRegistry} and installs a
     * {@link ReadinessEvaluator} built from the {@link ReadinessProbeRegistry} onto the
     * coordinator, so runtime {@code /ready-for-traffic} evaluates the live probe set
     * independently of the one-time warmup tasks. Uses the scheduler captured at
     * {@code createCoordinator}.
     *
     * @param warmupRegistry the registered one-time startup warmup tasks
     * @param probeRegistry  the registered runtime readiness probes
     * @param config         supplies the warmup timeout budget
     * @param onReady        published exactly once when the warmup barrier completes
     */
    public void startWarmup(WarmupRegistry warmupRegistry, ReadinessProbeRegistry probeRegistry,
        ProxyConfig config, Runnable onReady) {
        LifecycleScheduler active = scheduler;
        if (active == null) {
            throw new IllegalStateException("startWarmup requires createCoordinator to run first");
        }
        startWarmup(warmupRegistry, probeRegistry, config, active, onReady);
    }

    /**
     * Same as {@link #startWarmup(WarmupRegistry, ProxyConfig, Runnable)} but with an
     * explicit scheduler; primarily for tests that drive the loop synchronously.
     */
    public void startWarmup(WarmupRegistry warmupRegistry, ProxyConfig config,
        LifecycleScheduler scheduler, Runnable onReady) {
        startWarmup(warmupRegistry, new ReadinessProbeRegistry(), config, scheduler, onReady);
    }

    /**
     * Same as {@link #startWarmup(WarmupRegistry, ReadinessProbeRegistry, ProxyConfig, Runnable)}
     * but with an explicit scheduler; primarily for tests that drive the loop synchronously.
     *
     * @param warmupRegistry the registered one-time startup warmup tasks
     * @param probeRegistry  the registered runtime readiness probes
     * @param config         supplies the warmup timeout budget
     ...
     */
    public void startWarmup(WarmupRegistry warmupRegistry, ReadinessProbeRegistry probeRegistry,
        ProxyConfig config, LifecycleScheduler scheduler, Runnable onReady) {
        WarmupBarrier barrier = new WarmupBarrier(warmupRegistry.tasks());
        ReadinessEvaluator evaluator = new ReadinessEvaluator(probeRegistry.probes(),
            DEFAULT_DEPENDENCY_FAILURE_THRESHOLD, barrier::hasCompletedWarmup);
        ProxyLifecycleCoordinator coordinator = coordinatorRef.get();
        if (coordinator != null) {
    ...
    private void driveWarmupTick(WarmupBarrier barrier, LifecycleScheduler scheduler,
